...
`.trim();
    ...
    await sendEmail({
        to: studentEmail,
        subject: `✅ Session Booked – ${formattedDate} at ${timeSlot} | KU Wellness Connect`,
        html,
        text,
    });
}
...
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
  <title>Welcome to KU Wellness Connect</title>
</head>
<body style="margin:0;padding:0;background:#f4f6fb;font-family:'Segoe UI',Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f6fb;padding:32px 0;">
    <tr>
      <td align="center">
        <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.08);">
          <tr>
            <td style="background:linear-gradient(135deg,#1a3a5c 0%,#2563eb 100%);padding:32px 40px;text-align:center;">
              <h1 style="margin:0;color:#ffffff;font-size:22px;font-weight:700;letter-spacing:0.5px;">
                🎓 Welcome to KU Wellness Connect
              </h1>
            </td>
          </tr>
          <tr>
...
`.trim();
    ...
    await sendEmail({
        to: email,
        subject: `🎓 Welcome to KU Wellness Connect`,
        html,
        text,
    });
}
...
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
  <title>Reset Your Password - KU Wellness Connect</title>
</head>
<body style="margin:0;padding:0;background:#f4f6fb;font-family:'Segoe UI',Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f6fb;padding:32px 0;">
    <tr>
...
`.trim();
    ...
    await sendEmail({
        to: email,
        subject: `🔒 Reset Your Password | KU Wellness Connect`,
        html,
        text,
    });
}
                ...
                <tr>
                  <td style="padding:16px 28px;">
                    <p style="margin:0;font-size:13px;color:#1e40af;line-height:1.5;">
                      ℹ️ You will receive another email once your counselor confirms or updates your appointment.
                    </p>
                  </td>
                </tr>
              </table>
...
<body style="margin:0;padding:0;background:#f4f6fb;font-family:'Segoe UI',Arial,sans-serif;">
  ...
  `.trim();
    ...
    const results = await Promise.allSettled([
        sendEmail({
            to: studentEmail,
            subject: `📋 Booking Submitted – ${formattedDate} at ${timeSlot} | KU Wellness Connect`,
            html: studentHtml,
        ...
        sendEmail({
            to: counselorEmail,
            subject: `📥 New Booking Request from ${studentName} – ${formattedDate} | KU Wellness Connect`,
            html: counselorHtml,
...
<body style="margin:0;padding:0;background:#f4f6fb;font-family:'Segoe UI',Arial,sans-serif;">
  ...
  `.trim();
    ...
    await sendEmail({
        to: counselorEmail,
        subject: `✅ Session Confirmed – ${studentName} on ${formattedDate} | KU Wellness Connect`,
        html,
      ...
      <td align="center">
        <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.08);">
          <tr>
            <td style="background:linear-gradient(135deg,#1a3a5c 0%,#2563eb 100%);padding:32px 40px;text-align:center;">
              <h1 style="margin:0;color:#ffffff;font-size:22px;font-weight:700;letter-spacing:0.5px;">
                🛡️ KU Wellness Connect — Admin Alert
              </h1>
          ...
          <tr>
            <td style="padding:36px 40px;">
              ...
              <p style="font-size:13px;color:#9ca3af;line-height:1.6;margin:0;">
                Kenyatta University — KU Wellness Connect
              </p>
            </td>
          </tr>
          <tr>
...
`.trim();
    ...
    await sendEmail({
        to: adminEmail,
        subject: `⏳ New Counselor Registration Pending Approval — ${counselorName} | KU Wellness Connect`,
        html,
...
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
  <title>Account Approved — KU Wellness Connect</title>
</head>
<body style="margin:0;padding:0;background:#f4f6fb;font-family:'Segoe UI',Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f6fb;padding:32px 0;">
    <tr>
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.08);">
          ...
          <tr>
            <td style="padding:36px 40px;">
              <p style="margin:0 0 8px;font-size:16px;color:#374151;">Hi <strong>${counselorName}</strong>,</p>
              <p style="margin:0 0 24px;font-size:15px;color:#6b7280;line-height:1.6;">
                Great news! Your counselor account on <strong>KU Wellness Connect</strong> has been reviewed and <strong style="color:#16a34a;">approved</strong> by the system administrator.
              </p>
              <p style="margin:0 0 28px;font-size:15px;color:#6b7280;line-height:1.6;">
...
`.trim();
    ...
    await sendEmail({
        to: counselorEmail,
        subject: `✅ Your Counselor Account Has Been Approved | KU Wellness Connect`,
        html,
        text: `Hi ${counselorName},\n\nYour counselor account on KU Wellness Connect has been approved by the administrator.\n\nYou can now log in here: ${loginUrl}\n\nWelcome aboard!`,
    });
}
