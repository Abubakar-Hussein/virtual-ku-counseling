...
export default function AppointmentsHistoryPage() {
        ...
        try {
            ...
            if (data && data.length > 0) {
                const formatted = data.map((a: any) => {
                    const entry: any = {
                        StudentName: a.studentId?.name || 'Unknown', StudentEmail: a.studentId?.email || 'Unknown',
                        CounselorName: a.counselorId?.name || 'Unknown',
                        AppointmentDate: new Date(a.date).toLocaleDateString(), TimeSlot: a.timeSlot,
                    };
                ...
                openSigModal(formatted, `Counseling Appointments History — ${statusLabel}`, `${dateRange.start} → ${dateRange.end}`);
                showToast(`Appointments report ready — sign and print! (${formatted.length} records)`, 'success');
            } else { showToast('No appointments found for the selected filters.', 'error'); }
        } catch { showToast('Failed to generate appointments report.', 'error'); }
    ...
    if (apptStatus !== 'all') hiddenCols.push('status');
            ...
            <main className="dashboard-content page-transition">
                ...
                <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                        <div style={{ fontSize: '2rem', width: 52, height: 52, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 14, background: 'rgba(245,158,11,0.15)', border: '1px solid rgba(245,158,11,0.3)' }}>📅</div>
                        <div>
                            <h1 style={{ fontSize: '1.6rem', fontWeight: 800, margin: 0 }}>Appointments History</h1>
                            <p style={{ color: 'var(--text-secondary)', margin: 0, fontSize: '0.9rem' }}>Filter, preview, and print appointment reports.</p>
                        </div>
                    </div>
                    <NotificationBell />
                </header>
                ...
                <div className="glass" style={{ padding: 24, marginBottom: 28 }}>
                    <p style={{ fontSize: '0.78rem', fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 16 }}>Filter Appointments By</p>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                                {STATUS_OPTIONS.map(s => (
                                    <button key={s.value} onClick={() => setApptStatus(s.value)} style={{
                                        ...
                                        fontSize: '0.72rem', fontWeight: apptStatus === s.value ? 700 : 400, cursor: 'pointer', transition: 'all 0.2s',
                                    }}>{s.label}</button>
                                ))}
                            </div>
                        </div>
                    ...
                    <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
                        <button className="btn-primary" onClick={fetchPreview} disabled={previewLoading} style={{ fontSize: '0.85rem', background: 'linear-gradient(135deg, var(--ku-gold), #d97706)' }}>
                            {previewLoading ? '⏳ Loading...' : '👁 Preview Data'}
                        </button>
                        <button onClick={printReport} disabled={printing} style={{
                            padding: '10px 20px', borderRadius: 12, border: '1px solid var(--ku-gold)', background: 'transparent',
                            color: 'var(--ku-gold)', fontWeight: 600, fontSize: '0.85rem', cursor: printing ? 'not-allowed' : 'pointer',
                ...
                {previewData.length > 0 && (
                    <div className="glass" style={{ padding: 24, overflowX: 'auto' }}>
                        <h2 style={{ fontSize: '1.1rem', fontWeight: 700, marginBottom: 16 }}>Preview: Appointments History ({previewData.length} records)</h2>
