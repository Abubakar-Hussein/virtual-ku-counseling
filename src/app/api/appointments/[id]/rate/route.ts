...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        const cProfile = await CounselorProfile.findOne({ userId: appointment.counselorId });
        if (cProfile) {
            const currentTotal = cProfile.totalRatings || 0;
            const currentAvg = cProfile.averageRating || 0;
            
            const newTotal = currentTotal + 1;
            const newAvg = ((currentAvg * currentTotal) + rating) / newTotal;
            
            cProfile.totalRatings = newTotal;
            cProfile.averageRating = parseFloat(newAvg.toFixed(1)); // Store 1 decimal place
            await cProfile.save();
        }
